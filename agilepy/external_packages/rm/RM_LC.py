...
def read_r(CONT: str, TEMPO: str, SX: float, DX: float, BKG_SX: float, BKG_DX: float, GRB_SX: float, GRB_DX: float, FLAG: str, NAME: str, FLAG_PLOT: str):
...
def main():
    # Check arguments
    if len(sys.argv) != 12:
        print(f"Usage: python RM_LC.py [contact] [T0 in UTC or OBT] [START] [STOP] [BKG START] [BKG STOP] [GRB START] [GRB STOP] [\"ND\" or \"D\" for NON-DETRENDED or DETRENDED] [NAME] [\"8RM\" or \"3RM\" for 8 ratemeters or 3 ratemeters]")
        print(f"Example: python RM_LC.py 080686 2022-10-28T13:16:27 -100 100 -100 -90 -1 3 ND GRB221028A 8RM")
        sys.exit(1)
    ...
    SX     = float(sys.argv[3])
    DX     = float(sys.argv[4])
    BKG_SX = float(sys.argv[5])
    BKG_DX = float(sys.argv[6])
    GRB_SX = float(sys.argv[7])
    GRB_DX = float(sys.argv[8])
    FLAG   = str(sys.argv[9])
    NAME   = str(sys.argv[10])
    FLAG_PLOT = str(sys.argv[11])
    
    # Print
    print(f"CONTACT={CONT}")
    print(f"T0={TEMPO}. Burst name={NAME}.")
    print(f"Time range plot = [{SX},{DX}] s.")
    print(f"Time range background= [{BKG_SX},{BKG_DX}] s.")
    print(f"Time range signal = [{GRB_SX},{GRB_DX}] s.")
    print(f"Data Flag={FLAG}. Plot Flag={FLAG_PLOT}.")
    
    # Run Script
    read_r(CONT, TEMPO, SX, DX, BKG_SX, BKG_DX, GRB_SX, GRB_DX, FLAG, NAME, FLAG_PLOT)
